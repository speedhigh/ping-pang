...
<template>
  <div
    class="overlay-root fixed inset-0 z-50 flex flex-col bg-[var(--panel-bg)] pt-[env(safe-area-inset-top,0px)] pb-[env(safe-area-inset-bottom,0px)]"
  >
    <div class="flex min-h-0 flex-1 flex-col items-stretch justify-end px-4 pt-4 sm:items-center sm:justify-center">
      <div class="flex min-h-0 w-full max-w-md flex-1 flex-col sm:max-h-[min(92dvh,calc(100dvh-env(safe-area-inset-top)-env(safe-area-inset-bottom)-2rem))] sm:flex-none">
        ...
        <button
          type="button"
          class="mt-4 mb-4 shrink-0 w-full rounded-full py-3 text-sm font-semibold text-white transition active:brightness-90 sm:mb-0"
