...
推送 `main_github` 分支后，GitHub Actions 会自动构建并部署到 `gh-pages` 分支。

**访问地址：** https://speedhigh.github.io/ping-pang/

**首次启用**（只需做一次）：仓库 **Settings → Pages → Build and deployment**：

1. **Source** 选 **Deploy from a branch**（不要选 GitHub Actions）
2. **Branch** 选 `gh-pages`，目录选 `/ (root)`
3. 保存后等 1～2 分钟

推送 `main_github` 或到 **Actions** 页手动 **Run workflow** 会自动更新站点。

**大陆用户说明：** jsDelivr（`cdn.jsdelivr.net`）在大陆已基本不可用，会出现 502 或「Failed to fetch version info」，请勿使用。请直接用上面的 GitHub Pages 地址；若较慢或打不开，见下方 Cloudflare Pages。

### 方式二：Cloudflare Pages（大陆用户推荐）

Cloudflare 在亚太有边缘节点，大陆访问通常比 `github.io` 更稳定。

1. 代码已在 GitHub
2. 打开 [Cloudflare Dashboard](https://dash.cloudflare.com/) → **Workers & Pages** → **Create** → **Pages** → **Connect to Git**
3. 选择 `speedhigh/ping-pang` 仓库，分支选 `main_github`
4. 构建设置：Framework **None**，Build command `pnpm build`，Build output directory `dist`
5. 部署完成后获得 `https://ping-pang.pages.dev`（或自定义子域）
